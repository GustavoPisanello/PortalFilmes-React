    ...
    return (
        <>
            <div key={id} className="flex flex-col text-center justify-center items-center gap-y-4 flex-shrink-0 relative">
                ...
                <button className="absolute top-1 -right-2 p-2 bg-white rounded-full transition ease-in-out duration-300 z-10 transform hover:scale-125"
                ...
                <Link to={`/movies/${id}`} className='px-6 py-2 transition ease-in-out duration-300 bg-primary_color hover:bg-white text-white hover:text-primary_color rounded-xl font-[Raleway]'>Ver detalhes</Link>
            </div>
        </>
    )
}